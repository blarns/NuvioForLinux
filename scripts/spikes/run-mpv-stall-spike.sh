#!/usr/bin/env bash
# Compile and run MpvStallSpike.kt: does a stalled network source show up as loading, and does
# the log say why? The spike serves the given LOCAL file over a credit-limited HTTP server it
# starts itself, so the stall is exact and nothing external is needed.
#
#   run-mpv-stall-spike.sh <file>
#
...
OUT="${TMPDIR:-/tmp}/mpv-stall-spike"
...
java -cp "$COMPILER_CP" org.jetbrains.kotlin.cli.jvm.K2JVMCompiler \
  ...
  "$MPV_SRC/MpvEngineOptions.kt" "$MPV_SRC/MpvSoftwareRenderer.kt" \
  "$REPO/scripts/spikes/MpvStallSpike.kt"

echo "running..."
exec java -cp "$OUT:$APP_CP" com.nuvio.app.desktop.mpv.MpvStallSpikeKt "$@"
