#!/usr/bin/env bash
# Compile and run MpvSwRenderSpike.kt against the REAL software render path.
#
#   run-mpv-swrender-spike.sh <file-or-url>
#
...
OUT="${TMPDIR:-/tmp}/mpv-swrender-spike"
...
java -cp "$COMPILER_CP" org.jetbrains.kotlin.cli.jvm.K2JVMCompiler \
  ...
  "$REPO/scripts/spikes/MpvSwRenderSpike.kt"

echo "running..."
exec java -cp "$OUT:$APP_CP" com.nuvio.app.desktop.mpv.MpvSwRenderSpikeKt "$@"
